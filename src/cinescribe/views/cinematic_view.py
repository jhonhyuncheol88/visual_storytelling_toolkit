...
from ..repository.cinematic_repository import CinematicRepository
...
class CinematicView(QWidget):
    ...
    VisualPromptView와 유사한 단일 JSON(Text) 문서 편집/저장/내보내기 구성.
    저장은 프로젝트 DB 내 CinematicBoard 테이블(단일 행)에 기록된다.
    """

    def __init__(self) -> None:
        super().__init__()

        self._repo: CinematicRepository | None = None

        root = QVBoxLayout(self)
        root.setContentsMargins(12, 12, 12, 12)

        toolbar = QHBoxLayout()
        title = QLabel("시네마틱 (JSON)")
        ...
        self._editor.setPlaceholderText("시네마틱 보드용 JSON 코드를 입력하세요…")
    ...
    def _ensure_repo(self) -> None:
        ...
        if self._repo is None or self._repo._db_path != db_path:
            self._repo = CinematicRepository(db_path)
        self._editor.setEnabled(True)
    ...
    def _on_load(self) -> None:
        ...
        if board is None:
            ...
            if board.format == "json":
                ...
                self._editor.setPlainText(board.content)
        self._status.setText("불러오기 완료: 시네마틱 보드")
    ...
    def _on_export(self) -> None:
        ...
        if not self._repo:
            return
        default_name = "cinematic_board.json"
